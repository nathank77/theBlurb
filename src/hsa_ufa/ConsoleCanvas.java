...
public class ConsoleCanvas extends JPanel implements Runnable, KeyListener {

  private static final long serialVersionUID = 1L;
  private String title;
  private hsa_ufa.Console parent;
  ...
  private int fontHeight, fontBase, fontWidth, fontSize;
  private int cursorRow = 1, cursorCol = 1;
  private boolean cursorFlashing = false;
  private int flashSpeed = 5; // speed is in frames (see framesPerSecond above)
  private int flashCount = 0;
  private boolean cursorVisible = false;

  // Text output variables - adapted from hsa
  ...
  // Keyboard Buffer & Input Variables - adapted from hsa
  ...
  // The key currently down & last key pressed
  protected int currentKeyCode = Console.VK_UNDEFINED;
  protected int lastKeyCode = currentKeyCode;
  protected char currentKeyChar = (char) Console.VK_UNDEFINED;
  protected char lastKeyChar = currentKeyChar;
  protected final int numKeyCodes = 256;
  protected boolean[] keysDown = new boolean [numKeyCodes];
  ...
  protected void drawImage(Image img, int x, int y, ImageObserver io)
  {
    boolean success = false;
    synchronized(buffer)
    {
      ...
      for (int i = 0 ; i < 1000 & !success ; i++)
      {
        success = g.drawImage (img, x, y, io);
        try
        {
          Thread.sleep (1);
        }
  ...
  protected void drawImage(Image img, int x, int y, int width, int height, ImageObserver io)
  {
    boolean success = false;
    synchronized(buffer)
    {
      ...
      for (int i = 0 ; i < 1000 & !success ; i++)
      {
        success = g.drawImage (img, x, y, width, height, io);
        try
        {
          Thread.sleep (1);
        }
  ...
  public char getChar (boolean cursor)
  {
    while (kbdBufferHead == kbdBufferTail)
    {
      try
      {
        parent.setTitle (title + " - Waiting for input");
        if (cursor)
          cursorOn();
        else
          cursorOff();
